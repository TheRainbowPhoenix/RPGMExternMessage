...
var $externMessage =
{
	...
	getValue: function(key) {
		...
		if (key && key.replace && key.includes("\n")) {
			key = key.replaceAll("\n", "\\n");
			swapBreak = true;
		}
		...
		if(!line) {
			if (key && key.startsWith && (key.startsWith("\\P") || key.startsWith("\\V"))) {
				line = [key];
			} else {
				return undefined;
			}
	...
	parseLine: function(line) {
		...
		while (currentPos < line.length) {
			...
			if (line.startsWith('"""', currentPos)) {
				const endPos = line.indexOf('"""', currentPos + 3);
				if (endPos !== -1) {
					currentField = line.substring(currentPos + 3, endPos);
					currentPos = endPos + 3;
				} else {
					// If no closing triple quotes found, take rest of line
					currentField = line.substring(currentPos + 3);
					currentPos = line.length;
				}
				row.push(currentField.trim());
				
				// Skip to next field after comma
				currentPos = line.indexOf(',', currentPos);
			...
			if (char === '"') {
				...
				if (endPos !== -1) {
					currentField = line.substring(currentPos + 1, endPos);
					currentPos = endPos + 1;
				} else {
					// If no closing quote found, take rest of line
					currentField = line.substring(currentPos + 1);
					currentPos = line.length;
				}
				row.push(currentField.trim());
			...
			const nextComma = line.indexOf(',', currentPos);
			if (nextComma !== -1) {
				currentField = line.substring(currentPos, nextComma);
				currentPos = nextComma + 1;
			} else {
				currentField = line.substring(currentPos);
				currentPos = line.length;
			}
			row.push(currentField.trim());
		}

		// Clean up empty fields at the end
...
// Example usage:
// const csvText = 'Name,Comment\nJohn,"""Hello, world\nThis is \"triple quoted\"!""",Tester';
// console.log(CsvImportor.parseFromCSV(csvText));
...
(function() {
	...
	Window_Base.prototype.drawText = function(text, x, y, maxWidth, align) {
		var result = $externMessage.getValue(text);
		if (result !== undefined) {
			text = result
		}
	...
	Window_Base.prototype.drawTextEx = function(text, x, y, width) {
		...
		_Window_Base_drawTextEx.apply(this, arguments);
	};

	const _TextManager_message = TextManager.message
	TextManager.message = function(messageId) {
		let message = _TextManager_message.apply(this, arguments);

		var result = $externMessage.getValue(message);
		if (result !== undefined) {
			return result
		} else {
			return message
	...
	globalThis.TranslationManager = {
		// Translation method using $externMessage.getValue
		translate: function (text) {
			return new Promise((resolve) => {
				const result = $externMessage.getValue(text);
				resolve(result !== undefined ? result : text);
			});
		},

		translateIfNeed: function(glossaryName, callback) {
			const result = $externMessage.getValue(glossaryName);

			callback(result !== undefined ? result : glossaryName)
		},
	
		// Public method to return a translation promise
		getTranslatePromise: function (text) {
...
(function() {
	...
	Game_Interpreter.prototype.rollbackList = function() {
		...
		if (this._originalList) {
			// 現在のコマンドの元のインデックス位置を検索
			// 検索の際はshallow equalで全検索する。
			// 全検索する理由は文章の置き換えでスクリプトのみ実行など文章が消える場合もあり、
			// 必ずしもoriginalList上での位置が現在位置よりも前という保証はないため。
			var current = this._list[this._index];
			var originalIndex = this._originalList.indexOf(current);
			// 無いと思うが何らかの原因で見つからない場合はrollbackを行わずにそのまま継続させる。
			if (originalIndex < 0) {
				console.warn("ExternalMessage warn:", "元のインデックス位置を復元できませんでした。\nこのまま処理を継続します。この時文章の再置換が行われない可能性があります。");
				return;
			}
	...
	{
		// Bypass translate for one-shot
		if (item && item.parameters && item.parameters.length == 1) {
			let fast_swap = $externMessage.getValue(item.parameters[0])
			if (fast_swap) {
				item.parameters[0] = fast_swap;
				return [item]; 
			}
		}
