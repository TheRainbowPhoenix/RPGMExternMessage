...
function normalizeJapaneseChunk(raw: string): string | null {
  const candidate = raw
    .replace(/\\+/g, "")
    .replace(/^[`"'\\/,\s]+|[`"'\\/,\s]+$/g, "")
    .trim();

  if (!candidate || !hasJapanese(candidate)) return null;
  return candidate;
}

function collectJapaneseChunks(text: string): Set<string> {
  const results = new Set<string>();
  for (const match of text.matchAll(JAPANESE_SEGMENT)) {
    const normalized = normalizeJapaneseChunk(match[0]);
...
async function extractJapaneseStrings(
  ...
  const seen = collectJapaneseChunks(content);
  const lines = content.split(/\r?\n/);
  const quickMatches = [...content.matchAll(JAPANESE_SEGMENT)].length;
  if (quickMatches === 0) {
    console.log("No Japanese segments detected during quick scan.");
  }
  ...
  for (const line of lines) {
    ...
    for (const match of quoteMatches) {
      ...
      for (const chunk of collectJapaneseChunks(decoded)) seen.add(chunk);
    }

    const backtickMatches = line.matchAll(/`([^`]*)`/g);
    for (const match of backtickMatches) {
      for (const chunk of collectJapaneseChunks(match[1])) seen.add(chunk);
    }

    const commentIndex = line.indexOf("//");
    if (commentIndex >= 0) {
      for (const chunk of collectJapaneseChunks(line.slice(commentIndex + 2))) {
        seen.add(chunk);
      }
    }
    ...
    if (line.length > 255) {
      ...
      for (const part of parts) {
        for (const chunk of collectJapaneseChunks(part)) {
          seen.add(chunk);
        }
      }
    }
  ...
  if (seen.size === 0) {
    console.log(
      `No Japanese strings found in plugins file (read ${content.length} bytes from ${pluginsPath}).`,
    );
    return;
  }
  console.log(`Found ${seen.size} Japanese strings in plugins file.`);
